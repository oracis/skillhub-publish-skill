...
## GitHub 建仓与推送（**不用向用户要 PAT**）

本机**已有可用的长期 GitHub 凭据**，直接拿来建仓 + push 即可。

### 凭据在哪

`gh` CLI 本身**没登录**（`gh auth status` 报 not logged in，`~/.config/gh/hosts.yml` 里只有 `user: oracis` 没有 `oauth_token`），
**别看错地方**。真正的 token 在 git 的 credential store：
...
> ⚠️ `-c` 必须放在子命令 `push` **之前**。写成 `git push ... -c http.proxy=` 会报 usage 错误（git 会把它当 push 的参数）。

### 验证（别信本地 ref）

本地 `git log origin/main` 可能因 packed-refs 未刷新报
`fatal: ambiguous argument 'origin/main'`，**这不代表 push 失败**。以 GitHub API 为准：
